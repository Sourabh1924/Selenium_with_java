...
public class Travel_by_price {
	...
	public Travel_by_price(WebDriver driver,ExtentTest logger,ExtentReports report1) {
		this.driver = driver;
		this.logger = logger;
		...
		case "wanna get away":
			...
			break;
				...
				Collections.sort(al);
						for(WebElement we:List1)
						{
							if(al.get(al.size()-1).equals(Integer.parseInt((we.getText().replace("$", "")))))
							{
								we.click();
									try 
									{
