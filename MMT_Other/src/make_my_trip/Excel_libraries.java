...
public class Excel_libraries {
	...
	static String Folder_path = Function_libraries.Folder_path;
	static String Excel_path =  Function_libraries.Excel_path;
	static Workbook WB;
...
//------------------------------------------Create Excel file------------------------------------------
 ...
 //------------------------------------------For data access--------------------------------------------
 public static String Read(String Attri_name) throws Throwable
     {
		FileInputStream fin=new FileInputStream(System.getProperty("user.dir")+"\\src\\make_my_trip\\Excel_data\\MMT_data.xls");    
		WB=WorkbookFactory.create(fin);
		Sheet sh = WB.getSheet("Sheet1");
		int Row_cnt = sh.getLastRowNum();	
